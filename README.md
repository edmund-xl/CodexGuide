...
实战案例采用工具实测型写法，包含任务背景、环境与材料、操作剧本、过程证据、结果样例、踩坑记录、风险边界和可复用任务单。
...
案例页展示工具实测型复盘结构、证据清单和风险边界。

![案例页截图](assets/screenshots/recipe-deploy-diagnosis.png)
...
Recipes use a tool-tested retrospective format with task background, environment and materials, operating script, evidence trail, result sample, pitfalls, risk boundaries, and a reusable work order.
...
The recipe page shows the tool-tested retrospective structure, evidence checklist, and risk boundaries.

![Recipe page screenshot](assets/screenshots/recipe-deploy-diagnosis.png)
...
- `recipes/`: 13 tool-tested practical recipes
