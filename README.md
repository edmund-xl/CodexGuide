...
实战案例采用实测复盘写法，包含输入材料、运行环境、操作剧本、实测材料包、现场记录、过程证据、结果样例、失败与修正、风险边界、验收标准和可复用任务单。每个案例都会生成可打开的输入任务单、证据 CSV、结果片段、验收 runbook 和证据看板。
...
Recipes use a tool-tested retrospective format with input materials, run environment, operating script, lab artifact pack, run log, evidence trail, result sample, failures and corrections, risk boundaries, acceptance criteria, and a reusable work order. Each recipe generates an openable input brief, evidence CSV, result sample, acceptance runbook, and evidence board.
