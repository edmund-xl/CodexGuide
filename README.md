...
实战案例采用实测复盘写法，包含输入材料、运行环境、操作剧本、过程证据、结果样例、失败与修正、风险边界、验收标准和可复用任务单。
...
部署诊断案例展示任务状态面板、证据表、验收命令和失败修正记录。
...
Recipes use a tool-tested retrospective format with input materials, run environment, operating script, evidence trail, result sample, failures and corrections, risk boundaries, acceptance criteria, and a reusable work order.
...
The deployment diagnosis recipe shows the task status panel, evidence table, acceptance commands, and correction record.
